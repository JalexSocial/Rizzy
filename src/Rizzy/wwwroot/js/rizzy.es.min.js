(function() {
  function S(m, p, y) {
    p && y && (m = m.replaceAll(y, p));
    const g = new DOMParser();
    try {
      const w = g.parseFromString(m, "text/html");
      if (w)
        return w.querySelectorAll("script, style, link").forEach((M) => {
          M.getAttribute("nonce") !== p && M.remove();
        }), w.documentElement.outerHTML;
    } catch {
    }
    return "";
  }
  htmx.registerExtension("rizzy-nonce", {
    htmx_after_request: function(m, p) {
      let y = htmx.config.documentNonce ?? htmx.config.inlineScriptNonce;
      y || (console.warn("rizzy-nonce extension loaded but no nonce found for document. Inline scripts may be blocked."), y = "");
      let g = p?.ctx?.response?.headers?.get("HX-Nonce");
      if (!g) {
        const w = p?.ctx?.response?.headers?.get("content-security-policy");
        if (w) {
          const N = w.match(/(style|script)-src[^;]*'nonce-([^']*)'/i);
          N && (g = N[2]);
        }
      }
      return g ?? (g = ""), typeof p?.ctx?.text == "string" && (p.ctx.text = S(p.ctx.text, y, g)), !0;
    }
  });
})();
(function() {
  ...
  class p extends HTMLElement {
    connectedCallback() {
      ...
      f.parentNode?.removeChild(f), f.childNodes.forEach((b) => {
        if (b instanceof HTMLTemplateElement) {
          const s = b.getAttribute("blazor-component-id");
          s && D(s, b.content);
        }
  ...
  htmx.registerExtension("rizzy-streaming", {
    init: function(c) {
      S = c, htmx.blazorSwapSsr == null && (customElements.get("blazor-ssr-end") === void 0 && customElements.define("blazor-ssr-end", p), htmx.blazorSwapSsr = g);
    },
    htmx_after_init: function() {
      return htmx?.process(document.body), !0;
    },
    htmx_before_request: function(c, f) {
      const b = S.getSwapSpecification(c), s = f.ctx.fetch || window.fetch;
      return f.ctx.fetch = async function(e, r) {
        const t = await s(e, r);
        if (!t.body)
          return t;
        const n = t.body.getReader(), i = new TextDecoder();
        let a = null;
        const o = "ctr" + crypto.randomUUID();
        for (; ; ) {
          ...
          const u = i.decode(l, { stream: !0 });
          a || (a = document.createElement("div"), a.id = o, M(c, a.outerHTML, b), a = document.getElementById(o) ?? a), M(a, u, { ...b, swapStyle: "beforeend", settleDelay: 0 });
        }
        return f.ctx.swap = "none", f.ctx.text = "", new Response("", { status: t.status, statusText: t.statusText, headers: t.headers });
      }, !0;
    }
  });
  function y(c) {
    if (c && c.nodeType === Node.COMMENT_NODE) {
      let f = c.parentNode;
      for (; f !== null; ) {
        if (f === document.head)
          return !0;
  ...
  function g(c, f, b) {
    const s = w(c, f, "ssr" + crypto.randomUUID()), e = document.createElement("div");
    e.appendChild(b), M(s, e.innerHTML), N(s);
  }
  function w(c, f, b) {
    ...
    let e = c.nextSibling;
    for (; e && e !== f; )
      s.appendChild(e), e = c.nextSibling;
    return c.parentNode.insertBefore(s, f), s;
  }
  function N(c) {
    if (c.parentNode) {
      for (; c.firstChild; )
  ...
  function M(c, f, b) {
    const s = S.getTarget(c);
    S.swap(s, f, b ?? S.getSwapSpecification(c));
  }
  function D(c, f) {
    const b = T(c);
    if (b) {
      const { startMarker: s, endMarker: e } = b;
      if (m = !y(s), m)
        g(s, e, f);
      else {
        const r = e.parentNode, t = new Range();
        for (t.setStart(s, s.textContent.length), t.setEnd(e, 0), t.deleteContents(); f.childNodes[0]; )
          r.insertBefore(f.childNodes[0], e);
      }
    }
  }
  function T(c) {
    const f = `bl:${c}`, b = document.createNodeIterator(document, NodeFilter.SHOW_COMMENT);
    let s = null;
    for (; (s = b.nextNode()) && s.textContent !== f; )
      ;
    if (!s)
      return null;
    const e = `/bl:${c}`;
    let r = null;
    for (; (r = b.nextNode()) && r.textContent !== e; )
      ;
    return r ? { startMarker: s, endMarker: r } : null;
  }
})();
var F = { exports: {} }, O;
function P() {
  return O || (O = 1, (function(S, m) {
    (function(y, g) {
      S.exports = g();
    })(self, () => (
      /******/
      (() => {
        ...
        p.d = (s, e) => {
          for (var r in e)
            p.o(e, r) && !p.o(s, r) && Object.defineProperty(s, r, { enumerable: !0, get: e[r] });
        }, p.o = (s, e) => Object.prototype.hasOwnProperty.call(s, e), p.r = (s) => {
        ...
        }, D = ["input", "select", "textarea"], T = function(s) {
        ...
        function c(s, e) {
          ...
          return s.form.querySelector(T("[name=".concat(t, "]")));
        }
        var f = (
          /** @class */
          /* @__PURE__ */ (function() {
            function s() {
              this.required = function(e, r, t) {
                ...
                if (n === "checkbox" || n === "radio") {
                  for (var i = Array.from(r.form.querySelectorAll(T("[name='".concat(r.name, "'][type='").concat(n, "']")))), a = 0, o = i; a < o.length; a++) {
                    var d = o[a];
                ...
                return new Promise(function(V, L) {
                  ...
                  if (t.type && t.type.toLowerCase() === "post") {
                    ...
                    for (var U in i)
                      A.append(U, i[U]);
                    I.open("post", C), I.setRequestHeader("Content-Type", "application/x-www-form-urlencoded"), I.send(E);
                  } else
                    I.open("get", C + "?" + E), I.send();
                  I.onload = function(k) {
                    if (I.status >= 200 && I.status < 300) {
                      var q = JSON.parse(I.responseText);
                      V(q);
                    } else
                      L({
                        status: I.status,
          ...
          (function() {
            ...
            return s.prototype.addProvider = function(e, r) {
              ...
              for (var u = 0, h = d; u < h.length; u++)
                for (var o = h[u], C = Array.from(o.querySelectorAll("[data-valmsg-for]")), x = 0, v = C; x < v.length; x++) {
                  var a = v[x];
                  r.call(this, o, a);
                }
              ...
              var d = function(l) {
                if (l.indexOf("-") === -1) {
                  for (var u = Object.keys(t).filter(function(V) {
                    return V !== l && V.indexOf(l) === 0;
                  }), h = {
                    error: t[l],
                    params: {}
                  }, C = (l + "-").length, x = 0; x < u.length; x++) {
                    var v = t[u[x]], E = u[x].substr(C);
                    h.params[E] = v;
                  }
                  r[l] = h;
                }
              ...
              if (d ? (o.push(r), this.options.addNoValidate ? (this.logger.log("Setting novalidate on form", e), e.setAttribute("novalidate", "novalidate")) : this.logger.log("Not setting novalidate on form", e)) : this.logger.log("Form input for UID '%s' is already tracked", r), !this.formEvents[a]) {
                var l = null, u = function(C, x) {
                  return l || (t.shouldValidate(C) ? (l = t.getFormValidationTask(a), C && t.preValidate(C), t.logger.log("Validating", e), l.then(function(v) {
                    return g(t, void 0, void 0, function() {
                      var E;
                      return w(this, function(V) {
                        switch (V.label) {
                          case 0:
                            return this.logger.log("Validated (success = %s)", v, e), x ? (x(v), [2, v]) : (E = new CustomEvent("validation", {
                              detail: { valid: v }
                            }), e.dispatchEvent(E), [4, new Promise(function(L) {
                              return setTimeout(L, 0);
                            })]);
                          case 1:
                            return V.sent(), this.handleValidated(e, v, C), [2, v];
                        }
                      });
                    });
                  }).catch(function(v) {
                    return t.logger.log("Validation error", v), !1;
                  }).finally(function() {
                    l = null;
                  })) : Promise.resolve(!0));
                };
                ...
                var h = function(C) {
                  for (var x = t.formInputs[a], v = 0, E = x; v < E.length; v++) {
                    var V = E[v];
                    t.resetField(V);
                  }
                  t.renderSummary();
                ...
                var a = function(h, C) {
                  return g(r, void 0, void 0, function() {
                    ...
                    return w(this, function(V) {
                      switch (V.label) {
                        case 0:
                          if (x = this.validators[n], !x)
                            return [2, !0];
                          if (!e.dataset.valEvent && h && h.type === "input" && !e.classList.contains(this.ValidationInputCssClassName))
                            return [2, !0];
                          this.logger.log("Validating", { event: h }), V.label = 1;
                        case 1:
                          return V.trys.push([1, 3, , 4]), [4, x()];
                        case 2:
                          return v = V.sent(), C(v), [2, v];
                        case 3:
                          return E = V.sent(), this.logger.log("Validation error", E), [2, !1];
                        case 4:
                          return [
                            2
                            /*return*/
              ...
              var t = Array.from(e.querySelectorAll(T('[data-val="true"]')));
              ...
              if (this.highlight(e, this.ValidationInputCssClassName, this.ValidationInputValidCssClassName), e.form)
                for (var i = e.form.querySelectorAll(T('[name="'.concat(e.name, '"]'))), n = 0; n < i.length; n++) {
              ...
              if (this.unhighlight(e, this.ValidationInputCssClassName, this.ValidationInputValidCssClassName), e.form)
                for (var n = e.form.querySelectorAll(T('[name="'.concat(e.name, '"]'))), t = 0; t < n.length; t++) {
                  ...
                  return w(this, function(E) {
                    switch (E.label) {
                      case 0:
                        ...
                        o = 0, E.label = 1;
                      case 1:
                        return o < i.length ? (a = i[o], a in n ? (d = a, l = r[d], u = this.providers[d], u ? (this.logger.log("Running %s validator on element", d, e), h = u(e.value, e, l.params), C = !1, x = l.error, typeof h != "boolean" ? [3, 2] : (C = h, [3, 5])) : (this.logger.log("aspnet-validation provider not implemented: %s", d), [3, 6])) : [3, 6]) : [3, 7];
                      case 2:
                        return typeof h != "string" ? [3, 3] : (C = !1, x = h, [3, 5]);
                      case 3:
                        return [4, h];
                      case 4:
                        v = E.sent(), typeof v == "boolean" ? C = v : (C = !1, x = v), E.label = 5;
                      case 5:
                        if (!C)
                          return this.addError(e, x), [2, !1];
                        E.label = 6;
                      case 6:
                        return o++, [3, 1];
                      case 7:
...
var _ = P();
document.body.attributes.__htmx_antiforgery || (document.addEventListener("htmx:config:request", (S) => {
  const m = S.detail?.ctx?.request;
  if (!m || m.method?.toUpperCase() === "GET") return;
  const p = htmx.config?.antiforgery;
  if (!p) return;
  const { headerName: y, requestToken: g, formFieldName: w } = p;
  if (!y && !w) {
    console.warn("Antiforgery configuration is missing both headerName and formFieldName. Token not added.");
    return;
  }
  w && m.body && typeof m.body.has == "function" && m.body.has(w) || (y ? m.headers[y] = g : w && m.body && typeof m.body.append == "function" && m.body.append(w, g));
}), document.addEventListener("htmx:after:request", (S) => {
  if (S.detail.boosted) {
    const m = S.detail.ctx.text, p = "meta[name=htmx-config]", y = m.indexOf('<meta name="htmx-config"'), g = m.indexOf(">", y) + 1, w = m.indexOf("</meta>", g);
    if (y > -1) {
      let N = "";
      if (w > -1) {
        const T = m.indexOf("/", y);
        T > -1 && T < g ? N = m.substring(y, g) : N = m.substring(y, m.indexOf("</meta>") + 7);
      } else
        N = m.substring(y, g);
      const M = /content="([^"]*)"/, D = N.match(M);
      if (D && D[1]) {
        const T = D[1], c = document.querySelector(p), f = "antiforgery";
        try {
          htmx.config[f] = JSON.parse(T)[f];
        } catch (s) {
          console.error("Error parsing htmx-config JSON:", s);
          return;
        }
        const b = document.createElement("meta");
        b.setAttribute("name", "htmx-config"), b.setAttribute("content", T), c.replaceWith(b);
      }
    }
  }
}), document.body.attributes.__htmx_antiforgery = !0);
let H = new _.ValidationService();
H.bootstrap({ watch: !0 });
const R = {
  validation: H
};
